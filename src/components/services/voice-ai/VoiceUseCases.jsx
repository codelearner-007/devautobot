...
const config = [
  ...
  { icon: GraduationCap, iconColor: 'text-primary',     iconBg: 'bg-primary/10 border-primary/25',        glow: 'hsl(var(--primary) / 0.25)', ring: 'hsl(var(--primary))', tagBg: 'bg-primary/10 border-primary/25 text-primary' },
  { icon: Wrench,        iconColor: 'text-amber-400',   iconBg: 'bg-amber-400/10 border-amber-400/25',   glow: 'rgba(251,191,36,0.25)',      ring: '#fbbf24',              tagBg: 'bg-amber-400/10 border-amber-400/25 text-amber-400' },
  { icon: Briefcase,     iconColor: 'text-pink-400',    iconBg: 'bg-pink-500/10 border-pink-400/25',     glow: 'rgba(244,114,182,0.25)',     ring: '#f472b6',              tagBg: 'bg-pink-400/10 border-pink-400/25 text-pink-400' },
];
...
export default function VoiceUseCases() {
  return (
    <ServiceUseCases
      useCases={useCases}
      config={config}
      description="Every industry has leads going cold. We deploy AI calling agents that respond in seconds."
    />
  );
}
