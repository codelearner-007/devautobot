...
const config: ServiceUseCaseConfig[] = [
  ...
  { icon: Users,     iconColor: 'text-emerald-400', iconBg: 'bg-emerald-400/10 border-emerald-400/25',glow: 'rgba(52,211,153,0.25)',      ring: '#34d399',              tagBg: 'bg-emerald-400/10 border-emerald-400/25 text-emerald-400' },
  { icon: Target,    iconColor: 'text-amber-400',   iconBg: 'bg-amber-400/10 border-amber-400/25',   glow: 'rgba(251,191,36,0.25)',      ring: '#fbbf24',              tagBg: 'bg-amber-400/10 border-amber-400/25 text-amber-400' },
  { icon: Globe,     iconColor: 'text-primary',     iconBg: 'bg-primary/10 border-primary/25',        glow: 'hsl(var(--primary) / 0.25)', ring: 'hsl(var(--primary))', tagBg: 'bg-primary/10 border-primary/25 text-primary' },
];

export default function AvatarUseCases() {
  return (
    <ServiceUseCases
      useCases={useCases}
      config={config}
      description="Every industry needs scalable video content. We build AI avatars that deliver it without the studio."
    />
  );
}
